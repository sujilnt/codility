...
const arrayMinMax = (arr)=>{
  ...
  let totalSum = arr.reduce((c,a,b)=>c+a);
	let smallestElement = totalSum-arr[0], largestElement = totalSum-arr[0];
  for(let i =1,currentValue, len= arr.length;i< len; i++){
	  currentValue = totalSum-mainArray[i];
  	  smallestElement = currentValue < smallestElement ? currentValue : smallestElement ;
  	  largestElement = largestElement > currentValue ? largestElement: currentValue;
  }
